public  class TwentyTLHandler extends BanknotHandler{

    private final int DENOMINATION =20;
    private int stock;

    public TwentyTLHandler(int stock){
        this.stock = stock;
    }
    @Override
    protected void process(WithdravalRequest request) {
        if (request.getRemainingAmount() >= DENOMINATION && stock >0){
            ...
            System.out.println("20 TL" + minValue + " adet verildi. Kalan tutar: " + request.getRemainingAmount() +" Banknot stoğu: "+stock);
        }
    }
